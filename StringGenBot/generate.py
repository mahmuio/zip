...
buttons_ques = [
    ...
    [
        InlineKeyboardButton("بايلجرام بوت ❤️‍🔥 💎", callback_data="pyrogram_bot"),
...
gen_button = [
    [
        InlineKeyboardButton(text=" [{اضـعـط لـبـدا اسـتـخـراج الـكـود}] ٖ 💎 ", callback_data="generate")
    ]
]
...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    ...
    if api_id_msg.text == "/skip":
        ...
        try:
            ...
            return
        api_hash_msg = await bot.ask(user_id, "» حسنا قم بارسال الٍـ API_HASH", filters=filters.text)
    ...
    if not is_bot:
        await msg.reply(" انتظر سوف نرسل كود لحسابك بالتلجرام❤️‍🔥 .")
    else:
    ...
    if not is_bot:
        ...
        try:
            ...
            try:
                two_step_msg = await bot.ask(user_id, " ارسٍَِل البسورد الخاص بحسابك.", filters=filters.text, timeout=300)
            except TimeoutError:
    ...
    if telethon:
        ADXG25 = client.session.save()
    else:
        ADXG25 = await client.export_session_string()
